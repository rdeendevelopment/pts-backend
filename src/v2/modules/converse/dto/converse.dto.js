...
function toConversationDto(conversation, participant, extras = {}) {
  ...
  return {
    ...
    lastReadSequence: Number(participant?.lastReadSequence || 0),
    lastReadMessageId: participant?.lastReadMessageId ? String(participant.lastReadMessageId) : null,
...
function toMessageDto(message, senderName = '') {
  ...
  return {
    ...
    mentionAll: Boolean(doc.mentionAll),
